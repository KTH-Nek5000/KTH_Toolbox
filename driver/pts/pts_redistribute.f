!> @file pts_redistribute.f
!! @ingroup pts
!! @brief Set of subroutines to redistribute pionts among processors
!! according to point-element map.
!! @author Adam Peplinski
!! @date May 31, 2016
!=======================================================================
!> @brief Point redistribution timing.
!! @ingroup pts
!! @details Test interpolation time for single variable before and after
!!  point redistribution. This routine assumes physical point coordinates
!!  to be provided and stored in pts_crd declared in PTSD include file.
!! @param[in]  nptimb    maximal allowed point imbalance
!! @param[inout] urbuf     user defined real buffer
!! @param[in]    lurbuf    number of real variables
!! @param[in]    lpbuf     buffer length
!! @param[in]    ifutrans  do we perform user buffer transfer
!! @note This routine perform interpolation of x velocity component for
!!  timing test.
      subroutine pts_timing(nptimb,urbuf,lurbuf,lpbuf,ifutrans)
      ...
      include 'INPUT_DEF'
      include 'INPUT'
      include 'PTSD'          ! point time series for 2D statistics

!     argument list
      integer nptimb            ! allowed point imbalance
      integer lurbuf,lpbuf
      real urbuf(lurbuf,lpbuf)
      logical ifutrans

!     local variables
      real fieldout(LHIS) ! collected field values
...
!     poitn-to-element mappting
      call findpts(pts_ihpts,pts_rcode,1,pts_proc,1,pts_elid,1,
     &     pts_rst,NDIM,pts_dist,1,pts_crd(1,1),NDIM,
     &     pts_crd(2,1),NDIM,pts_crd(3,1),NDIM,pts_npts)
...
!     interpolation; single velocity component
!     VX
      call findpts_eval(pts_ihpts,fieldout,1,
     &     pts_rcode,1,pts_proc,1,pts_elid,1,
     &     pts_rst,NDIM,pts_npts,VX)
...
!     timing
      ...
      call pts_rdst(nptimb,urbuf,lurbuf,lpbuf,ifutrans)
...
!=======================================================================
!> @brief Redistribute points.
!! @ingroup pts
!! @details Redistribute points according to point-to-element map.
!!  This routine assume findpts was alleready called and pts_proc array
!!  is filled.
!! @param[in]    nptimb    maximal allowed point imbalance
!! @param[inout] urbuf     user defined real buffer
!! @param[in]    lurbuf    number of real variables
!! @param[in]    lpbuf     buffer length
!! @param[in]    ifutrans  do we perform user buffer transfer
!! @remarks This routine uses global scratch space SCRMG, SCRNS
      subroutine pts_rdst(nptimb,urbuf,lurbuf,lpbuf,ifutrans)
...
!     prepare mapping
!     this routine does not update pts_npts
      call pts_map_all(mid,lptn,pts_npts,pts_npoints,nptimb)

!     save local point number for user buffer transfer
      itmp1 = pts_npts

!     transfer module specified point data
      if (NIO.eq.0) write(6,*) 'Module specified point data transfer.'
!     fill in redistribution array
!     integer
      do il=1,pts_npts
         ibuf(1,il) = mid(il)
         ibuf(2,il) = pts_ipts(il)
         ibuf(3,il) = pts_proc(il)
         ibuf(4,il) = pts_elid(il)
         ibuf(5,il) = pts_rcode(il)
      enddo
...
!     real
      if (IF3D) then
         do il=1,pts_npts
            itmp = (il-1)*NDIM
            rbuf(1,il) = pts_crd(1,il)
            rbuf(2,il) = pts_crd(2,il)
            rbuf(3,il) = pts_crd(3,il)
            rbuf(4,il) = pts_rst(itmp+1)
            rbuf(5,il) = pts_rst(itmp+2)
            rbuf(6,il) = pts_rst(itmp+3)
         enddo
      else
         do il=1,pts_npts
            itmp = (il-1)*NDIM
            rbuf(1,il) = pts_crd(1,il)
            rbuf(2,il) = pts_crd(2,il)
            rbuf(3,il) = pts_rst(itmp+1)
            rbuf(4,il) = pts_rst(itmp+2)
         enddo
      endif

!     redistribute point data
!     this routine updates pts_npts
      call pts_transfer(rbuf,lrbuf,ibuf,libuf,lptn,pts_npts)

!     copy arrays back
!     integer
      do il=1,pts_npts
         pts_ipts(il) = ibuf(2,il)
         pts_proc(il) = ibuf(3,il)
         pts_elid(il) = ibuf(4,il)
         pts_rcode(il) = ibuf(5,il)
      enddo
...
!     real
      if (IF3D) then
         do il=1,pts_npts
            itmp = (il-1)*NDIM
            pts_crd(1,il) = rbuf(1,il)
            pts_crd(2,il) = rbuf(2,il)
            pts_crd(3,il) = rbuf(3,il)
            pts_rst(itmp+1) = rbuf(4,il)
            pts_rst(itmp+2) = rbuf(5,il)
            pts_rst(itmp+3) = rbuf(6,il)
         enddo
      else
         do il=1,pts_npts
            itmp = (il-1)*NDIM
            pts_crd(1,il) = rbuf(1,il)
            pts_crd(2,il) = rbuf(2,il)
            pts_rst(itmp+1) = rbuf(3,il)
            pts_rst(itmp+2) = rbuf(4,il)
         enddo
      endif

!     transfer user specified point data
      if (ifutrans) then
         if (NIO.eq.0)
     $        write(6,*) 'User specified point data transfer.'
!     check buffer size
         if (LHIS.gt.lpbuf) then
            if (NIO.eq.0)
     $       write(6,*) 'Error: pts_rdst; insufficient user buffer size'
            call exitt
         endif
!     transfer buffer
         itmp2 = min(lpbuf,lptn)
         call pts_transfer(urbuf,lurbuf,mid,1,itmp2,itmp1)
      endif

      return
      end
!=======================================================================
!> @brief Point data transfer.
!! @ingroup pts
!! @param[inout]  rbuf   real buffer
!! @param[in]     lrbuf  number of real variables
!! @param[inout]  ibuf   integer buffer
!! @param[in]     libuf  number of integer variables
!! @param[in]     lpts   buffers length
!! @param[inout]  npts   local number of points
...
!     stamp logs
      if (NIO.eq.0) then
         write(6,*)
         write(6,*) 'Point data redistribution; pts_transfer_min:'
...
!=======================================================================
!> @brief Point-process mapping
!! @ingroup pts
!! @details Genereate point-process mapping for point redistribution.
!!   This routine leaves local number of points npts unchanged.
!! @param[inout] mid       target proc id; global ordering
!! @param[in]    lpts      array size
!! @param[in]    npts      local number of points
!! @param[in]    npoints   global number of points
!! @param[in]    nptimb    max allowed imbalance
!! @remarks This routine uses global scratch space SCRVH
...
!=======================================================================
!> @brief Point-process mapping for given process set.
!! @ingroup pts
!! @details Genereate point-process mapping for point redistribution
!!   for given process set. This routine leaves local number of points
!!   npts unchanged.
!! @param[inout] mid       target proc id; global ordering
!! @param[in]   lpts      mid array size
!! @param[inout] istatus   node status
!! @param[inout] indasg    number of points asigned to the node
!! @param[in]    npts_plist list of processors used
!! @param[in]    lplist    npts_plist array size
!! @param[in]    npts      local number of points
!! @param[in]    nplist    number of processors in the list
!! @remarks This routine uses global scratch space SCRCH, SCREV, CTMP0,
!!   CTMP1, SCRSF
